	...
	function execute($args='', $default_output_template='<li>{link}</li>'){
		...
		if (0 < $options['limit']) {
			...
			if (!function_exists('get_post_type')) { 
				...
				$where[] = where_show_status($options['status']);
			}
	    ...
	    if ($results) {
			...
			//$items = ppl_sort_results($options, $results, 'recent-posts');	
		...
		return $output . sprintf("<!-- Recent Posts took %.3f ms -->", 1000 * (ppl_microtime() - $start_time));
	}
