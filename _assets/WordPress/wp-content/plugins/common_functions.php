...
function where_show_pages($show_pages) {
	if (function_exists('get_post_type')) {
		if ($show_pages === 'true') $sql = "post_type IN ('page', 'post')";
		else if ($show_pages === 'false') $sql = "post_type='post'";
		else if ($show_pages === 'but') $sql = "post_type='page'";
	} else {
...
function where_show_status($status) {
	$set = array();
	$status = (array) $status;
...
function xwhere_included_cats($included_cats) {
	global $wpdb, $wp_version;
	...
	$catarray = array_unique($catarray);
	$in_posts = "'" . implode("', '", $catarray) . "'";
	$whichcat = "$wpdb->term_taxonomy.taxonomy = 'category' ";
	$whichcat .= " AND $wpdb->term_taxonomy.term_id IN ($in_posts)";
return $whichcat;
...
function xxscore_fulltext_match($table_name, $weight_title, $titleterms, $weight_content, $contentterms, $weight_tags, $tagterms, $forced_ids='') {
	global $wpdb;
	$wsql = array();
