...
// Admin stuff for Recent Posts Plugin, Version 2.5.0.10

function recent_posts_option_menu() {
	add_options_page(__('Recent Posts Options', 'recent_posts_plugin'), __('Recent Posts', 'recent_posts_plugin'), 8, 'recent-posts', 'recent_posts_options_page');
}
...
function recent_posts_general_options_subpage(){
	...
	if (isset($_POST['update_options'])) {
		...
		$options = ppl_options_from_post($options, array('limit', 'skip', 'show_private', 'show_pages', 'status', 'age', 'omit_current_post', 'match_cat', 'match_tags', 'match_author'));
...
function recent_posts_other_options_subpage(){
	...
	if (isset($_POST['update_options'])) {
		...
		if (defined('POC_CACHE_4')) poc_cache_flush();
		// Fill up the options with the values chosen...
		$options = ppl_options_from_post($options, array('content_filter', 'stripcodes', 'date_modified'));
		update_option('recent-posts', $options);
		// Show a message to say we've done something
		echo '<div class="updated fade"><p>' . __('Options saved', 'recent_posts_plugin') . '</p></div>';
	} 
